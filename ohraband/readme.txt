...
Hra sa bude ovládať kombináciou myši a klávesnice a bude implementovaná ako ťahová hra.

- `start ...` vytvorí novú hru podľa zadaných parametrov (level, difficulty...) 
- `load [filename]` načíta hru do súboru
- `save [filename]` uloží hru do súboru
- `left-mouse-button` zvolí základňu pre útok/na ktorú zaútočiť 
- `help` zobrazí nápovedu
- `quit` ukončí program

Forma príkazov ešte nie je konečná, podľa vôle autora je možné, že počítačová myš nakoniec nebude nutná 
alebo naopak bude viac integrovaná pomocou tlačítok na obrazovke.

Z grafickej stránky sa hra bude funkčne podobať na obrázok vyššie ↑↑↑, pričom sa využije konzolové prostredie (s možnou podporou
c++ knižnice ncurses).

Ku hre bude dodaná aj zložka obsahujúca dopredu vygenerované mapy, ktoré bude možné spustiť ako jednotlivé levely.

Po spustení sa program dostane do triedy `CMainMenu` , kde čaká na príkaz start. Ten spustí triedu s vygenrovanou mapou a stavom hry `CGame` , ktorá sa stará
o priebeh hry. Trieda `CMap` sa stará o mapu a objekty v nej. Objekt v mape je instancia triedy `CSymbol`, z ktorej dedia triedy
`CRepresentative` strarajúci sa o jednotlivých reprezentantov, `COutpost` ktorý zastupuje základňu(podnik) a `CBlock` zastupujúci
neprejditeľné prekážky. Trieda `CAction` prevádzkuje jednotlivé pohyby jednotiek po mape. O hráčov sa stará abstraktná trieda
`CPlayer`, ktorá sa ďalej delí na `CHuman` ovládanú človekom a `CComputer` ovládanú počítačom.


# Polymorfizmus --------------------------------------------------------------------------------------

Polymorfizmus využívam v `CSymbol`, kde jednotlivé objekty v mape majú rôzne vlastnosti a spôsoby zobtrazenia podľa typu.
Konkrétne ide o funkcie `Show`, ktorá hovorí ako sa má na mape daný objekt zobraziť, `Walkable`, kde je odopoveďou na otázku
či je možné cez daný objekt prejsť true alebo false a `Type`, ktorý jednoducho vráti typ daného objektu na dotazovanej pozícii. 
Tento polymorfizmus by určite šiel naradiť vhodnou funkciou, avšak kedže bude slúžiť najmä pre porovnanie vlastností a typov
príde mi prehľadnejšie mať to uvedené v každej triede samostatne.
 
Polymorfizmus v abstraktnej triede `CPlayer` má metódu `MakeMove`, ktorá v `CHuman` bude čakať na input od hráča,
podľa čoho následne vyberie začiatočnú a koncovú základňu, u `CComputer` metóda `MakeMove` vygeneruje pohyb cez jedno z implementovaných AI.
